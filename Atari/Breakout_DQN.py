...
class Controller:
    ...
    def make_network(self):
        ...
        self.layer_1_weights = tf.Variable(tf.truncated_normal([4, self.HIDDEN1_SIZE],stddev=0.01))
        ...
        self.output_layer_weights = tf.Variable(tf.truncated_normal([self.HIDDEN2_SIZE, 2],stddev=0.01))
        self.output_layer_biases = tf.Variable(tf.constant(0.01, shape=[2]))
        ...
        self.action = tf.placeholder("float", [None,2])
        ...
        for episode in range(num_episodes):
            ...
            for step in range(self.MAX_STEPS):
                ...
                if(len(self.MEMORY) >= 50):
                    ...
                    action_list = np.zeros((self.MINIBATCH_SIZE,2))
    ...
    def preprocess_input(self, obs):
        # this function converts images to a black and white 80 x 80 numpy array
