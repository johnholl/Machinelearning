...
class Controller:
    ...
    def make_network(self):
        ...
        self.input = tf.placeholder("float", [None, self.STATE_SIZE])
        self.layer_1_weights = tf.Variable(tf.truncated_normal([self.STATE_SIZE, self.HIDDEN1_SIZE],stddev=0.01))
        ...
        self.output_layer_weights = tf.Variable(tf.truncated_normal([self.HIDDEN2_SIZE, self.ACTION_SIZE],stddev=0.01))
        self.output_layer_biases = tf.Variable(tf.constant(0.01, shape=[self.ACTION_SIZE]))
        ...
        self.action = tf.placeholder("float", [None,self.ACTION_SIZE])
        ...
        for episode in range(num_episodes):
            ...
            for step in range(self.MAX_STEPS):
                ...
                if(len(self.MEMORY) >= 50):
                    ...
                    action_list = np.zeros((self.MINIBATCH_SIZE,self.ACTION_SIZE))
                    ...
                    _, summary = self.session.run([self.train_operation, self.summary], feed_dict={
                        self.input: states,
            ...
            print("Training episode = {}, Total steps = {}, Last episode length = {}"
                  .format(episode, total_steps, steps))
